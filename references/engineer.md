...
1. 从 `[板书]-实时笔记.md` 中剪切阶段一至阶段二的完整内容
2. 使用 Write 工具创建 `[知识点]-[贴合技术主题的名称].md`（如"RAII与智能指针设计取舍"、"虚函数多态设计思想"），写入 frontmatter（含 previous/next/branch_of）+ 内容
3. 更新 `[板书]-实时笔记.md`：原位置替换为 `→ 已拆分：[[知识点-名称]]`
