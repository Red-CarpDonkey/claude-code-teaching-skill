...
当 topic 是编程语言/技术框架时，学科识别确认后增加一问：

> "你想以哪本书/教程为主线？没有的话我推荐 2-3 本主流教材，你选。"

确认后，教材的章节目录作为路线图。状态追踪增加 `textbook` 字段：

```json
"textbook": {"title": "教材名", "current_chapter": "Ch3", "chapters_completed": ["Ch1", "Ch2"]}
```

**每章仍然完整执行 7 阶段循环。** 教材保证知识覆盖的全面性，工程师模式保证学习的扎实程度。

### 按章节拆分

当使用教材路线图时，每完成一章：

1. 将该章的板书内容从 `[板书]-实时笔记.md` 中**提取**到 `[板书]-ChN-[章节名].md`
2. `[板书]-实时笔记.md` 清空（只保留新一章的初始标题），开始新一章
3. 将该章内所有知识点生成知识卡片，按学习链连接
4. 更新索引

### 提问归档

用户每次提出新问题时，立即生成 `[疑问]-[简短标题].md` 记录问题和解答。`.study/questions.md` 只做摘要 + 链接指向该疑问文件。
...
1. 使用 Write 工具创建 `[知识点]-[贴合技术主题的名称].md`（如"RAII与智能指针设计取舍"、"虚函数多态设计思想"），写入完整内容 + frontmatter（含 previous/next/branch_of）
