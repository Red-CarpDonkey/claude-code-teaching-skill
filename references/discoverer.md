...
发现者模式的7个阶段是五个元学习阶段的实例化：

| 元阶段 | 发现者阶段 |
|--------|-----------|
| **I 动因确立** | 阶段一：历史电报 |
| **II 本体解构** | 阶段二：原境思考 + 阶段三：跨学科工具箱 |
| **III 因果建模** | 阶段四：关键抉择 + 阶段五：推导共创 |
| **IV 辩证审视** | 阶段六：涟漪效应 |
| **V 范式融合** | 阶段七：现代视角 |
...
当用户选择了教材（见 SKILL.md 通用机制），教材章节作为学习路线图。每章完整执行 7 阶段循环。
...
1. 使用 Write 工具创建 `[知识点]-[贴合学习主题的名称].md`，写入完整内容 + frontmatter（含 previous/next/branch_of）。卡片内容以用户自己的话为核心
2. 使用 Edit 工具在 `[板书]-实时笔记.md` 中对应内容的**开头处**插入一行：`→ 详见：[[知识点-名称]]`
3. **板书原文保留不删除**。板书是完整的学习记录，知识卡片是结构化归档
4. 更新 `00-总览仪表盘/索引.md`：在当前主题行追加 `→ [[知识点-名称]]`
5. 更新 `.study/state.json`：`split_cards` 追加记录，`vault.last_card_file` 更新为当前卡片路径

**卡片命名必须贴合具体内容，不使用阶段编号。**
...
> **注意：CLI 中公式必须转自然语言。** 如说"p平方等于2乘以q平方"而非"$p^2 = 2q^2$"。
