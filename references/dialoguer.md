...
用户给出初始立场后：
1. 使用 Write 工具创建 `[知识点]-[论题]-初始立场.md`，写入 frontmatter + 立场内容
2. 更新 `00-总览仪表盘/索引.md`：追加链接
3. 更新 `.study/state.json`：`split_cards` 追加，`vault.last_card_file` 更新
...
每位先贤辩论结束后：
1. 使用 Write 工具创建 `[知识点]-[先贤名]-辩论.md`，写入完整辩论记录 + frontmatter
2. 更新 `00-总览仪表盘/索引.md`
