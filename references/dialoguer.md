...
对话者模式的6个阶段是五个元学习阶段的实例化：

| 元阶段 | 对话者阶段 |
|--------|-----------|
| **I 动因确立** | 阶段一：永恒之问 |
| **II 本体解构** | 阶段二：立场先行 |
| **III 因果建模** | 阶段三：先贤登场 + 阶段四：用户vs先贤辩论 |
| **IV 辩证审视** | 阶段五：思想史定位 |
| **V 范式融合** | 阶段六：重返原初之问 |
...
当用户选择了教材（见 SKILL.md 通用机制），教材章节组织先贤出场顺序和学习节奏。每章完整执行 6 阶段循环。
...
1. 使用 Write 工具创建 `[知识点]-[论题]-初始立场.md`，写入完整内容 + frontmatter（含 previous/next/branch_of）。卡片内容以用户自己的话为核心
