...
小讲座结构（非工程师模式遵循SKILL.md通用三步，工程师模式遵循五步实操模板）：
...
1. **这是什么**（30秒）：一句话说清核心直觉，不展开
2. **使用模板——穷举所有情况**（写入板书，CLI只告知路径）：按类别分组穷举该概念的所有使用方式（创建、操作、访问、判空、传参、边界、非法写法）。模板是参考手册级别的全面覆盖，用户拿到后不需要再查文档
3. **使用实例**（30秒）：2-3个具体例子，每个一句话场景 + 代码片段
4. **可运行实验代码**（写入板书，CLI不展示）：完整复制即跑的代码（30-50行），预期输出写注释里
5. **解决了什么痛点**（用户跑通代码后触发）：用"你刚才看到的 [行为]——这就是 [概念] 解决的核心痛点"句式，将亲历体验与抽象问题对接

**工程师模式痛点后置的原因**：编程概念的"为什么需要它"在没亲手用过之前是抽象的。让用户先写、先跑、先看到行为——然后回看"刚才那个坑，这就是它的解决方案"——此时痛点描述不再是被动接收的信息，而是对已有体验的命名。

**如果用户表示不理解**：立即降层——换更生活化的类比，降到更基础层面。永远准备好更浅的版本。
